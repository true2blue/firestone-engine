...
class PPT0(Base):
    ...
    def match_shape(self):
        ...
        open_percent = (open_p - pre_close) / pre_close * 100
        ...
        if not self.is_enable_auto() and percent > float(self.trade['params']['drop_percent']) * -1:
            return False
        if hasattr(self, 'start_monitor') or (self.is_enable_auto() and percent < self.get_auto_determine_buy_percent(open_percent)) or (drop_percent_from_open < 0 and abs(drop_percent_from_open) >= start_buy_line) or (low_limit >= target_p and low_limit >= Decimal(close) and self.trade['params']['buy_on_low_limit'] == '1'):
